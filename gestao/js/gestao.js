  ...
  function orderRowHTML(o) {
    return `
      <tr data-order-row="${escapeHtml(o.id)}" tabindex="0">
        ...
        <td>${escapeHtml(PAYMENT_LABELS[o.paymentMethod] || o.paymentMethod || '—')}</td>
    ...
    try {
      ...
      $('[data-order-drawer-body]').innerHTML = `
        <dl class="gestao-detail-grid">
          ...
          <div><dt>Pagamento</dt><dd>${escapeHtml(PAYMENT_LABELS[order.paymentMethod] || order.paymentMethod || '—')}</dd></div>
          <div><dt>Sincronização Bling</dt><dd>${order.blingSyncStatus === 'disabled' ? 'Desativada (nenhum pedido é criado no Bling ainda)' : escapeHtml(order.blingSyncStatus)}</dd></div>
        </dl>
        <h3>Itens</h3>
    ...
    return `
      <div class="gestao-product-card">
        ...
        <div class="gestao-product-info">
          ...
          ${p.variants.length ? `<p class="gestao-product-meta">${escapeHtml(p.variants.length)} variação(ões)</p>` : ''}
